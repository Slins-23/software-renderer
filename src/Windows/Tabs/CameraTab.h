...
public:	
	...
	*/
	...
	/*
	void initialize(SceneTab* scene_tab, Scene* current_scene) {
		//this->scene_tab->current_scene.camera = &current_scene->camera;
		//this->scene_tab->current_scene.camera = &scene_tab->current_scene.camera;

		this->menu_translation_speed = &scene_tab->menu_translation_speed;
		this->menu_rotation_speed = &scene_tab->menu_rotation_speed;
		this->real_translation_speed = &scene_tab->real_translation_speed;
		this->real_rotation_speed = &scene_tab->real_rotation_speed;
	}
	*/

	/*
	void update(Camera* camera) {
		this->scene_tab->current_scene.camera = camera;
	...
	*/
	...
	void draw() {
		...
		if (ImGui::BeginTable("Projection matrix", 4, ImGuiTableFlags_None)) {
		...
		if (ImGui::BeginTable("View matrix", 4, ImGuiTableFlags_None)) {
			for (uint8_t row = 1; row < 5; row++) {
				ImGui::TableNextRow();
				for (uint8_t col = 1; col < 5; col++) {
					ImGui::TableNextColumn();
